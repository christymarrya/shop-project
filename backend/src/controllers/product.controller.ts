...
export const createProduct = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    // Check unique SKU
    const existingSku = await dbQuery('SELECT id FROM products WHERE sku = ?', [sku]);
    ...
    logSecurityEvent('product_creation', `Product created: ${name} (SKU: ${sku})`, {
      actor,
      ipAddress,
      userAgent,
...
export const updateProduct = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (sku && sku !== originalProduct.sku) {
      const existingSku = await dbQuery('SELECT id FROM products WHERE sku = ? AND id != ?', [sku, id]);
      if (existingSku.length > 0) {
        return res.status(400).json({ error: 'A product with this SKU already exists' });
      }
    }
...
export const deleteProduct = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    logSecurityEvent('product_deletion', `Product deleted (ID: ${id}): ${product.name} (SKU: ${product.sku})`, {
      actor,
      ipAddress,
      userAgent,
